...
interface FileUploadProps {
    onFileProcessed: (content: string | Array<{ text: string; timestamp: string }>, file: File) => void;
}
    ...
    return (
        <div className="flex items-center">
            <label htmlFor="file-upload" className="relative cursor-pointer bg-transparent rounded-full font-medium text-gray-400 hover:text-indigo-400 focus-within:outline-none p-2 hover:bg-gray-700 transition-colors">
                ...
                <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept=".txt,.docx,.srt,.vtt" />
            </label>
        </div>
    );
};